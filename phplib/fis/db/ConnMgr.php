...
class Fis_Db_ConnMgr
{
    ...
    private static function _init($clusterName)
    {
        ...
        if(self::$_conf  === false || $clusterName == '')
        {
            ...
            Fis_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "ConnMgr", $clusterName, "init", "",
            ...
            && self::$_conf['retry_interval_s'] !== '')
        {
            self::$_dbData['status_manager'] 
                = new Fis_Db_StatusManFile(DATA_PATH.'/db/');
        }
        $className = self::$_conf['balance_strategy'] === '' ? 'Fis_Db_RandBalancer'
    ...
    */
    ...
    {	
        ...
        if($hookBeforeInit === false){
            ...
            //Fis_Log::warning(self::$_error['error'],self::$_error['errno']);
            Fis_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "ConnMgr", $clusterName, "getConn", "",
            	0, 0, 0, 0, 0, $clusterName, "", self::$_error['errno'], self::$_error['error']);
            return false;
        }
        if($hookBeforeInit != NULL){
            ...
            if(is_callable($hookBeforeInit)){
                ...
                //Fis_Log::warning(self::$_error['error'],self::$_error['errno']);
                Fis_Db_RALLog::warning(RAL_LOG_WARN, "ConnMgr", $clusterName, "getConn", "",
                	0, 0, 0, 0, 0, $clusterName, "", self::$_error['errno'], self::$_error['error']);
            }
        }     
...
//                         );
            ...
            for($i = 1; $i <= $conf['retry_times']; $i++)
            {
                ...
                Fis_Db_RALLog::warning(RAL_LOG_DEBUG, "ConnMgr", $clusterName, "getConn", "",

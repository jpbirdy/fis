...
class Fis_DB_SplitDB
{
    ...
    public function doSql($strSql, $fetchType = Fis_Db::FETCH_ASSOC, $bolUseResult = false)
    {
        ...
        if (NULL === $arrSql || $arrSql['err_no'] !== 0)
        {
            ...
            Fis_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "SplitDB", $this->_strDBName, 'transform', '', $this->_timer->getTotalTime(Fis_Timer::PRECISION_MS), 0, 0, 0, $this->_transTime, '', $strSql, self::$TRANSFORM_ERRNO, self::$TRANSFORM_ERROR);
            return false;
        }
        //some check before conneciton;
        if (Fis_DB_SplitDB::SPLITDB_SQL_SELECT === $arrSql['sql_type'] && $bolUseResult === true)
        {
            Fis_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "SplitDB", $this->_strDBName, 'check', '', $this->_timer->getTotalTime(Fis_Timer::PRECISION_MS), 0, 0, 0, 0, '', $strSql, self::$COMMON_ERRNO, "select should be store result");
            return false;
        }
        $arrSplitIndex = $this->analyseSqlTransResult($arrSql, Fis_DB_SplitDB::MAX_SQL_SPLIT_COUNT);
        if (!$arrSplitIndex || count($arrSplitIndex) <= 0)
        {
            Fis_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "SplitDB", $this->_strDBName, 'check', '', $this->_timer->getTotalTime(Fis_Timer::PRECISION_MS), 0, 0, 0, 0, '', $strSql, self::$COMMON_ERRNO, "splitindex count error");
            return false;
        }
        ...
        if ($this->_bolIsInTransaction)
        {
            ...
            {
                Fis_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "SplitDB", $this->_strDBName, 'check', '', $this->_timer->getTotalTime(Fis_Timer::PRECISION_MS), 0, 0, 0, 0, '', $strSql, self::$COMMON_ERRNO, "in transaction split count should be 1");
                return false;
            }
            ...
            if ($this->_intUseConnectionNum === 1)
            {
                ...
                {
                    Fis_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "SplitDB", $this->_strDBName, 'check', '', $this->_timer->getTotalTime(Fis_Timer::PRECISION_MS), 0, 0, 0, 0, '', $strSql, self::$COMMON_ERRNO, "in transaction connect should be the same");
                    return false;
                }
            }
            else
        ...
        switch ($fetchType)
        {
            ...
            default:
                Fis_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "SplitDB", $this->_strDBName, 'check', '', $this->_timer->getTotalTime(Fis_Timer::PRECISION_MS), 0, 0, 0, 0, '', $strSql, self::$COMMON_ERRNO, "fetch type error");
                return false;
        }
        ...
        foreach ($this->hkBeforeQ as $arrCallback)
        {
            ...
            {
                Fis_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "SplitDB", $this->_strDBName, 'query', '', $this->_timer->getTotalTime(Fis_Timer::PRECISION_MS), 0, 0, 0, 0, '', $strSql, self::$COMMON_ERRNO, "execute hooks before query fail");
                return false;
            }
        }
        ...
        foreach ($arrSql['sqls'] as $oneSql)
        {
            ...
            if (is_bool($res) || $res === NULL)
            {
                ...
                if (!$ok)
                {
                    ...
                    else if (($arrSql['sql_type'] === Fis_DB_SplitDB::SPLITDB_SQL_UPDATE || $arrSql['sql_type'] === Fis_DB_SplitDB::SPLITDB_SQL_DELETE) && $this->_arrMysql[$strDBInstanceName]->errno === Fis_DB_SplitDB::MYSQL_ERR_TABLE_NOT_EXIST && $arrSql['table_split_type'] === Fis_DB_SplitDB::SPLITDB_SPLIT_TYPE_RANGE)
                    {
                    ...
                    else
                    {
                        ...
                        Fis_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "SplitDB", $this->_strDBName, 'query', '', $this->_timer->getTotalTime(Fis_Timer::PRECISION_MS), 0, round($time / 1000), 0, 0, $strDBInstanceName, $oneSql['sql'], self::$QUERY_ERRNO, self::$QUERY_ERROR);
        ...
        {
            Fis_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "SplitDB", $this->_strDBName, 'query', '', $this->_timer->getTotalTime(Fis_Timer::PRECISION_MS), 0, round($this->_queryTime / 1000), 0, 0, '', $strSql, self::$QUERY_ERRNO, self::$QUERY_ERROR);
        ...
        Fis_Db_RALLog::notice(RAL_LOG_SUM_SUCC, "SplitDB", $this->_strDBName, 'query', '', $this->_timer->getTotalTime(Fis_Timer::PRECISION_MS), round($this->_connectTime / 1000), round($this->_queryTime / 1000), 0, round($this->_transTime / 1000), '', $strSql, count($arrSql['sqls']));

...
class Fis_Db
{
	...
	*/
    ...
    public function ralConnect($strName)
    {
    	...
    	if(!$this->isConnected){
    		Fis_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "Fis_DB", $this->$dbConf['dbname'], "connect", "{$this->$dbConf['host']}:{$this->$dbConf['port']}",
        ...
        */
        ...
        if(!is_string($sql))
        {
            ...
            if(!($sql instanceof Fis_Db_ISQL) || !($sql = $sql->getSQL()))
            {
				...
				Fis_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "Fis_DB", $this->dbConf['dbname'], "query", "{$this->dbConf['host']}:{$this->dbConf['port']}",
        ...
        Fis_Db_RALLog::notice(RAL_LOG_SUM_SUCC, "Fis_DB", $this->dbConf['dbname'], "query", "{$this->dbConf['host']}:{$this->dbConf['port']}",
        	round($this->lastCost/1000), 0, round($this->lastCost/1000), 0, 0, $this->dbConf['dbname'], $sql, 1);
...
//                echo "unknow type: $type_char\n";
	...
	*/
    public function prepare($query, $getRaw = false)
    {
		...
		Fis_Db_RALLog::notice(RAL_LOG_SUM_SUCC, "Fis_DB", $this->dbConf['dbname'], "prepare", "{$this->dbConf['host']}:{$this->dbConf['port']}",
			0, 0, 0, 0, 0, $this->dbConf['dbname'], $query, 1);

        if($stmt === false)
        {
        	Fis_Db_RALLog::warning(RAL_LOG_SUM_FAIL, "Fis_DB", $this->dbConf['dbname'], "prepare", "{$this->dbConf['host']}:{$this->dbConf['port']}",
...
*/
	...
	*/
    public function commit()
    {
		...
		Fis_Db_RALLog::notice(RAL_LOG_SUM_SUCC, "Fis_DB", $this->dbConf['dbname'], "commit", "{$this->dbConf['host']}:{$this->dbConf['port']}",
			0, 0, 0, 0, 0, $this->dbConf['dbname'], "", 0);
	...
	*/
    public function rollback()
	{
		...
		Fis_Db_RALLog::notice(RAL_LOG_SUM_SUCC, "Fis_DB", $this->dbConf['dbname'], "rollback", "{$this->dbConf['host']}:{$this->dbConf['port']}",
			0, 0, 0, 0, 0, $this->dbConf['dbname'], "", 0);
